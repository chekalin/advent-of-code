...
import static day5.AdvancedProgram.getNthDigitFromTheEnd;
import static org.assertj.core.api.Assertions.assertThat;

class AdvancedProgramTest {

    @Test
    void testInput() {
        int[] program = {3, 0, 99};
        AdvancedProgram.process(program, 1);
    ...
    void testMultipleInput() {
        ...
        AdvancedProgram.process(program, 100, 200);
    ...
    void testOutput() {
        int[] program = {4, 2, 99};
        int[] output = AdvancedProgram.process(program);
    ...
    void testMultiplicationByValue() {
        ...
        AdvancedProgram.process(program);
    ...
    void testMixedMultiplication() {
        ...
        AdvancedProgram.process(program1);
        assertThat(program1[0]).isEqualTo(16);

        int[] program2 = {1002, 2, 4, 0, 99};
        AdvancedProgram.process(program2);
    ...
    void testCase1() {
        ...
        assertThat(AdvancedProgram.process(program.clone(), 0)).isEqualTo(new int[]{0});
        assertThat(AdvancedProgram.process(program.clone(), 123)).isEqualTo(new int[]{1});
    }
    ...
    void testCase3() {
        ...
        assertThat(AdvancedProgram.process(program.clone(), 7)).isEqualTo(new int[]{999});
        assertThat(AdvancedProgram.process(program.clone(), 8)).isEqualTo(new int[]{1000});
        assertThat(AdvancedProgram.process(program.clone(), 9)).isEqualTo(new int[]{1001});
    }

    @Test
    void assignment() throws FileNotFoundException {
        int[] program = FileHelper.readProgramInput("day5/input.txt");
        int[] result1 = AdvancedProgram.process(program.clone(), 1);
        System.out.println("result1 = " + Arrays.toString(result1));
        int[] result2 = AdvancedProgram.process(program.clone(), 5);
        System.out.println("result2 = " + Arrays.toString(result2));
    }

    @Test
    void testGetNthDigitFromTheEnd() {
        assertThat(getNthDigitFromTheEnd(1, 0)).isEqualTo(1);
        assertThat(getNthDigitFromTheEnd(1, 1)).isEqualTo(0);
        assertThat(getNthDigitFromTheEnd(12, 1)).isEqualTo(1);
        assertThat(getNthDigitFromTheEnd(123, 0)).isEqualTo(3);
        assertThat(getNthDigitFromTheEnd(123, 1)).isEqualTo(2);
        assertThat(getNthDigitFromTheEnd(123, 2)).isEqualTo(1);
    }
}